 ...
 */
 ...
 public class Tree1Controller implements Initializable {
     @FXML
     private Pane myPane;
     @FXML 
      ...
      * Temp_level: the level of the tree use for drawing
  ...
  */
     ...
     public void getRightData(ActionEvent event){
        LeftOrRight = 1;
        leftButton.setSelected(false);
        change = true;
    }

    public void getLeftData(ActionEvent event){
        LeftOrRight = -1;
             ...
             for (int i = branchList.size()-1; i >=0;i--){
                 if (branchList.get(i).getLevel() == (Temp_level-1)){
                     ...
                     Node left = new Node(branchList.get(i).getDataX(), branchList.get(i).getDataY(), 
                                         lengthTemp, strokeWTemp, branchList.get(i).getAngle()+ (LeftOrRight*RAngleC),Temp_level);
